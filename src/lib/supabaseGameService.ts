...
const processGameMedia = async (game: Game, showId: string): Promise<Game> => {
  ...
  if (game.type === 'board') {
    const boardGame = { ...game } as any;
    const newCells = await Promise.all(
      boardGame.cells.map((row: any[], rowIdx: number) =>
        Promise.all(
          row.map(async (cell: any, colIdx: number) => {
            const newCell = { ...cell };
            if (cell.imageUrl && cell.imageUrl.startsWith('data:')) {
              newCell.imageUrl = await uploadMediaToStorage(
                cell.imageUrl, showId, `board-${rowIdx}-${colIdx}-img`
              );
            }
            if (Array.isArray(cell.imageUrls) && cell.imageUrls.length > 0) {
              newCell.imageUrls = await Promise.all(
                cell.imageUrls.map(async (url: string, i: number) => {
                  if (url && url.startsWith('data:')) {
                    return uploadMediaToStorage(url, showId, `board-${rowIdx}-${colIdx}-img-${i}`);
                  }
                  return url;
                })
              );
            ...
            if (cell.audioUrl && cell.audioUrl.startsWith('data:')) {
              newCell.audioUrl = await uploadMediaToStorage(
                cell.audioUrl, showId, `board-${rowIdx}-${colIdx}-audio`
              );
            }
            return newCell;
          })
